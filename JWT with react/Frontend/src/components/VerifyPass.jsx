...
export default function VerifyPass() {
    ...
    const handleVerifyPass = async (e) => {
        e.preventDefault();
        await axios.post("http://localhost:2005/verifyPass", formdata, {withCredentials: true}).then((res)=>{
            alert(res.data.msg)
            navigate("/")
        })
    }
            ...
            <div className="w-full max-w-md bg-white p-8 rounded-xl shadow-lg">
                ...
                <form onSubmit={handleVerifyPass}>
                    <div className="mb-4">
                        ...
                        <input type="text" name="otp" placeholder="Enter OTP" className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required onChange={handleChange} />
                    </div>
                    <div className="mb-4">
                    ...
                    <button type="submit" className="w-full cursor-pointer bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700 transition" >Reset Password</button>
                </form>
            </div>
        </div>
    )
